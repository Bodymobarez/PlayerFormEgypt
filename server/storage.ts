...
import { type User, type InsertUser, type Club, type InsertClub, type Assessment, type InsertAssessment, users, clubs, assessments } from "@shared/schema";
...
export class DbStorage implements IStorage {
  ...
  async getAssessmentsByClubId(clubId: string): Promise<Assessment[]> {
    return await db.select().from(assessments).where(eq(assessments.clubId, clubId));
  }

  async createAssessment(insertAssessment: InsertAssessment & { assessmentPrice: number }): Promise<Assessment> {
